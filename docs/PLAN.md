...
- **Submission:** Moodle wrapper PDF `MaRs-777-ex03.pdf` (separate from the article PDF); repo public or shared with rmisegal@gmail.com; correct Assignment 3 GitHub link.
...
src/agentic_latex_book/
  ...
  build/                 # planned: LaTeX -> PDF builder wrapper
...
The Moodle wrapper PDF (`MaRs-777-ex03.pdf`) is produced separately and manually from the official Word template; it is not generated by this pipeline (see §22).
...
- The **article/book PDF** (the main evaluated artifact) is produced by the pipeline and stored in `results/final_pdf/`.
- The **Moodle submission PDF** is a *separate* wrapper created manually from the official Word template, named **`MaRs-777-ex03.pdf`**, containing exercise number 03, group code MaRs-777, the Assignment 3 GitHub link, and the final self-assessment score. It is not generated by the pipeline (PRD §8 deliverable 10).
...
10. **Stage 12 — Documentation + submission:** complete `AI_WORKFLOW.md`, `PROMPTS.md`, `DECISIONS.md`, `QUALITY.md`, `SUBMISSION_CHECKLIST.md`; prepare `MaRs-777-ex03.pdf`; confirm repo sharing/link.
