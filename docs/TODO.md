...
- [x] Stage 9.2 — figure/graph generation (`scripts/generate_architecture.py`, `scripts/generate_graph.py` → `figures/architecture.png`, `figures/generation_time.png`; graph labelled illustrative). No compile and **no PDF**. (Committed and pushed: `6aa2cf1`.)
...
- [~] Stage 9.6 — final article PDF evidence (**in review before commit**): stale "future stages" wording fixed in `content/book.md` + abstract/discussion/conclusion chapters; clean rebuild re-validated (16 pages, all gates, stale-statement grep clean); `results/final_pdf/MaRs-777-article.pdf` (byte-identical copy, SHA256 recorded) + `validation_report.md` created. Not the Moodle wrapper; not committed yet.
...
- [~] 9.6 — final article PDF evidence (**in review before commit**). Human review blocked the first candidate: three stale "future stages" statements (abstract, discussion, conclusion) were corrected in `content/book.md` and the matching chapters (minimal consistency edits — no metric changed, no new claims). Clean rebuild from `899ebb8` + the fix re-passed all gates: 16 pages, clean log, 0 overfulls, 14/14 text-layer gates, 0 `??`, 16/16 pages rendered, stale-statement grep clean. `results/final_pdf/MaRs-777-article.pdf` is a `cmp`-verified byte-identical copy of `build/main.pdf` (SHA256 `b88fc2dd…ccd3ee`), accompanied by `results/final_pdf/validation_report.md` (with a consistency-fix note). The evidence PDF is not gitignored (`!results/**`) and is untracked pending commit approval. The Moodle wrapper `MaRs-777-ex03.pdf` is **not** created here.
