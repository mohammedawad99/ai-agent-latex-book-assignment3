...
- [~] Stage 9.5 — PDF validation and polish (**in review before commit**): rebuild is **16 pages, 0 overfulls, all 14 text-layer gates pass, no `??`**; key pages visually verified. Polish: 2.2 cm margins, 10pt, `\small` verbatim/table, `enumitem`, `\path`, biblatex URL penalties, figure widths. `build/main.pdf` still a build artifact only — **not final, not committed**.
