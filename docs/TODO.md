...
- [~] Stage 9.4 — first compile succeeded (XeLaTeX+biber; `build/main.pdf`, 19 pages, build artifact only — not committed). `texlive-lang-arabic` was required for `bidi.sty`; two minimal fixes (preamble package order, `\path` for long bibliography paths). **In review before commit.** Stage 9.5 validation/polish pending; **no final PDF yet**.
