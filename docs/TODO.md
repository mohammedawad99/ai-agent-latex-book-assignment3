...
- [~] Stage 9.2 — figure/graph generation created and accepted for commit (`scripts/generate_architecture.py`, `scripts/generate_graph.py` → `figures/architecture.png`, `figures/generation_time.png`; graph labelled illustrative). Scripts and PNGs exist; no compile and **no PDF**. (Commit hash recorded in the next docs update.)
