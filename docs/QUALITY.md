...
As of Stage 8C.7 (second full Gemini run — candidate content):

- **pytest is configured** and the offline test suite passes (`uv run pytest`): **86 tests passing** (including the Stage 8C.6 hardening checks). The test suite remains offline and never calls a model or `kickoff`.
...
- **Stage 8C.8.0 (plan only)** defines the content-QA cleanup: a future offline scanner (Stage 8C.8.1) will gate the cleaned content for no `example.com` / "Internal Publication" / unsupported metrics (185 minutes / 99.1% / 1.8M tokens) / unsupported GPT-4o claims, and for required topic, group/authors/date, mandatory PDF elements, and correctly-sourced runtime/token values. The cleaned content is a derived artifact under `latex_project/content/`; the run evidence stays immutable. No such scanner or cleaned content exists yet.
