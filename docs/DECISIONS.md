...
## D-030 — LaTeX Assembly Design: XeLaTeX + polyglossia/bidi, biblatex/biber, Generated Image Figures

- **Date:** 2026-06-12
- **Status:** Accepted (plan only — Stage 9.0; implementation in Stages 9.1–9.6; no `.tex`/`.bib`/image/PDF exists yet)
- **Context:** Stage 9.x must convert the committed cleaned Markdown (`latex_project/content/book.md`, commit `98a50f2`) into a polished ~15-page LaTeX article PDF with all mandatory elements (cover, ToC, chapters, headers/footers, image, Python-generated graph, table, formula, Hebrew–English BiDi section, linked bibliography). The engine/BiDi question was an open decision; this resolves it at the plan level.
- **Decision:**
  - **Engine: XeLaTeX** with `fontspec` + `polyglossia` (which loads `bidi` for Hebrew under XeTeX). Rationale: the polyglossia/bidi pairing is the mature XeTeX path for Hebrew RTL; full Unicode input; the cleaned content's committed BiDi snippet already targets exactly this stack with **DejaVu Sans**, which is verified installed and Hebrew-capable on this machine (`fc-list :lang=he` lists "DejaVu Sans"). LuaLaTeX (`babel`+`luabidi`) is the recorded fallback if XeLaTeX+bidi misbehaves. A font-availability check (DejaVu Sans present, else fail with a clear error) is a Stage 9.1 step — no font is assumed blindly.
  - **Prerequisite (system, not repo):** no TeX binaries exist yet (`which xelatex/lualatex/biber` is empty). Stage 9.1 requires installing TeX Live system packages (Ubuntu: `texlive-xetex`, `texlive-latex-extra`, `texlive-bibtex-extra`, `biber`, `fonts-dejavu`); this touches nothing in the repository.
  - **Bibliography: `biblatex` with the `biber` backend**, numeric style, entries in `latex_project/references.bib` transcribed from the cleaned content's bibliography only (7 external + 7 project-internal entries; no fake URLs, no `example.com`, no invented publications; project-internal items as `@misc` with repository paths in `note`). In-text `[n]` markers become `\cite{key}`; numbering is regenerated by biblatex (no manual numbers). Chosen over natbib/BibTeX for native Unicode, `@online`/`url` handling, and clean `hyperref` integration.
  - **Document class/layout:** `report`, 11pt, `a4paper`, `geometry` margins ~2.5 cm — sized for the 13–17 page target without padding. Minimal package set: `geometry`, `fontspec`, `polyglossia`, `amsmath`, `booktabs`, `graphicx`, `fancyhdr`, `listings` (+`xcolor` for listing style), `biblatex`, `hyperref` (loaded last). No `minted` (avoids shell-escape).
  - **Figures: generated image files, not TikZ**, for the first implementation. Both visual artifacts are produced by Python scripts under `latex_project/scripts/` writing to `latex_project/figures/`: the Chapter 3 architecture diagram (matplotlib boxes/arrows) and the Chapter 6 illustrative graph (the script already embedded in `book.md` §6.2). This keeps the preamble minimal, avoids TikZ+bidi interaction risks, and makes the "Python-generated" requirement provable. TikZ remains a later polish option. Scripts run via `uv run --with matplotlib python …` so `pyproject.toml`/`uv.lock` stay untouched (operational resolution of D-015; the matplotlib version used will be recorded in docs at Stage 9.2). This supersedes the older `assets/generated/`+`assets/figures/` naming in the Stage 10 checklist — figures live under `latex_project/figures/`.
  - **Headers/footers:** `fancyhdr` — short title "From PoC to Production" in the header, group code `MaRs-777` opposite, automatic page number in the footer; `plain` style on chapter openers. This implements (in Stage 9.1) what the cleaned content explicitly promises.
  - **Structure:** `latex_project/main.tex` + `preamble.tex`, `chapters/` (abstract + one file per chapter), `figures/`, `scripts/`, `references.bib`, gitignored `build/` output directory, and `README.md` (build prerequisites/commands). `latex_project/content/` remains the immutable cleaned source and is never edited by assembly.
  - **Source-to-LaTeX mapping:** cover page → `titlepage` (title, group, authors, course context, lecturer contact, date); `book.md`'s "Mandatory PDF Elements", "Note on headers and footers", and hand-written ToC sections are assembly instructions and are *not* typeset — the real ToC is `\tableofcontents`; `## n.` → `\chapter`, `### n.m` → `\section`; the two Markdown tables → `booktabs` tables; the two equations → `equation` environments; code blocks → `listings`; the ASCII pipeline diagram → the generated architecture figure; the BiDi snippet → an actually rendered Hebrew–English passage (`\begin{hebrew}…`); `[n]` markers → `\cite`. No manual page numbers or fake page markers anywhere.
- **Alternatives considered:** LuaLaTeX-first (kept as fallback; XeLaTeX matches the committed content snippet and the verified font); TikZ architecture diagram (more elegant, more first-compile risk); natbib/BibTeX (weaker Unicode/URL support); `minted` for code (needs shell-escape + pygments); adding matplotlib to `pyproject.toml` now (deferred — `uv run --with` avoids a dependency change until a deliberate decision).
- **Consequences:** Stage 9.1 can build the skeleton deterministically with no LLM/API involvement. PDF validation gates (page count 13–17, resolved citations, no `??`, BiDi renders, all mandatory elements present) are specified in `docs/QUALITY.md` and implemented at Stage 9.5. Until Stage 9.6 commits results, **no LaTeX, image, script, or PDF artifact exists**.
...
- LaTeX engine and BiDi package — **resolved at plan level by D-030** (XeLaTeX + polyglossia/bidi; LuaLaTeX fallback); confirm after first successful compile (Stage 9.4).
