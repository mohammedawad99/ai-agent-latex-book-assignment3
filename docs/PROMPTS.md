...
| 8A.1 | 2026-06-11 | Add Gemini provider support (offline) | AI coding assistant | Asked to extend the resolver to support `provider="gemini"` with `GEMINI_API_KEY`/`GOOGLE_API_KEY`, plus offline tests | Offline only; env-only secrets; no `.env` auto-load; no kickoff/LLM/API; no dependency added; no real run/evidence | `crew/llm.py`, `tests/test_crew_llm.py`, and docs | In review |
