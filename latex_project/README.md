...
**Status: skeleton + figures (Stages 9.1–9.2).** The chapter files contain
headings and TODO markers, not the full text. The two figures exist as of
Stage 9.2 (`figures/architecture.png`, `figures/generation_time.png`), both
deterministically generated by the scripts under `scripts/`. **No final PDF
exists yet and no compile has been attempted** (TeX Live is not installed).
Full chapter conversion (Stage 9.3), compilation (Stage 9.4), and PDF
validation (Stage 9.5) are future stages — see `docs/TODO.md` (Stage 9.0 plan)
and `docs/DECISIONS.md` (D-030).
